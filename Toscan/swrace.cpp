...
int patha(int r, int c, int pathN, int from, int tryesLeft, int lastTurn, int needToGo)
{
    ...
    if (me & ANELLO_WHITE)
    {
        ...
        {
        case UP:
            res = patha(r + 1, c, pathN, UP, 2, lastTurn + 1, lastTurn == 0 ? NOT_KNOWN : CURVATO);
            testNewDir = UP & DOWN;
            break;
        case DOWN:
            res = patha(r - 1, c, pathN, DOWN, 2, lastTurn + 1, lastTurn == 0 ? NOT_KNOWN : CURVATO);
            testNewDir = UP & DOWN;
            break;
        case LEFT:
            res = patha(r, c + 1, pathN, LEFT, 2, lastTurn + 1, lastTurn == 0 ? NOT_KNOWN : CURVATO);
            testNewDir = LEFT & RIGHT;
            break;
        case RIGHT:
            res = patha(r, c - 1, pathN, RIGHT, 2, lastTurn + 1, lastTurn == 0 ? NOT_KNOWN : CURVATO);
            testNewDir = LEFT & RIGHT;
            break;
    ...
    else if (me & ANELLO_BLACK)
    {
        ...
        {
            res = patha(r - 1, c, pathN, RIGHT, 2, 0, DRITTO);
            if (res > maxPoint && res > 0)
            {
                maxPoint = res + 1;
                newDir = (from & LEFT);
            }
            res = patha(r + 1, c, pathN, LEFT, 2, 0, DRITTO);
            if (res > maxPoint && res > 0)
            {
                maxPoint = res + 1;
                newDir = (from & RIGHT);
        ...
        {
            res = patha(r, c - 1, pathN, DOWN, 2, 0, DRITTO);
            if (res > maxPoint && res > 0)
            {
                maxPoint = res + 1;
                newDir = (from & UP);
            }
            res = patha(r, c + 1, pathN, UP, 2, 0, DRITTO);
            if (res > maxPoint && res > 0)
            {
                maxPoint = res + 1;
                newDir = (from & DOWN);
    ...
    else
    {
        ...
        case UP:
            if (needToGo != DRITTO)
            {
                res = patha(r + 1, c, pathN, UP, tryesLeft - 1, lastTurn + 1, NOT_KNOWN); //DOWN
                if (res > maxPoint && res > 0)
                {
                    maxPoint = res;
                    newDir = (from & DOWN);
            ...
            if (needToGo != CURVATO)
            {
                res = patha(r, c - 1, pathN, RIGHT, tryesLeft - 1, 0, NOT_KNOWN); // LEFT
                if (res > maxPoint && res > 0)
                {
                    maxPoint = res;
                    newDir = (from & LEFT);
                }
                res = patha(r, c + 1, pathN, LEFT, tryesLeft - 1, 0, NOT_KNOWN); //RIGHT
                if (res > maxPoint && res > 0)
                {
                    maxPoint = res;
                    newDir = (from & RIGHT);
        ...
        case DOWN:
            if (needToGo != DRITTO)
            {
                res = patha(r - 1, c, pathN, DOWN, tryesLeft - 1, lastTurn + 1, NOT_KNOWN); //UP
                if (res > maxPoint && res > 0)
                {
                    maxPoint = res;
                    newDir = (from & UP);
        ...
        case LEFT:
            if (needToGo != DRITTO)
            {
                res = patha(r, c + 1, pathN, LEFT, tryesLeft - 1, lastTurn + 1, NOT_KNOWN); //RIGHT
                if (res > maxPoint && res > 0)
                {
                    maxPoint = res;
                    newDir = (from & RIGHT);
            ...
            if (needToGo != CURVATO)
            {
                res = patha(r + 1, c, pathN, UP, tryesLeft - 1, 0, NOT_KNOWN); //DOWN
                if (res > maxPoint && res > 0)
                {
                    maxPoint = res;
                    newDir = (from & DOWN);
                }
                res = patha(r - 1, c, pathN, DOWN, tryesLeft - 1, 0, NOT_KNOWN); //UP
                if (res > maxPoint && res > 0)
                {
                    maxPoint = res;
                    newDir = (from & UP);
        ...
        case RIGHT:
            if (needToGo != DRITTO)
            {
                ...
                res = patha(r, c - 1, pathN, RIGHT, tryesLeft - 1, lastTurn + 1, NOT_KNOWN); //LEFT
                if (res > maxPoint && res > 0)
                {
                    maxPoint = res;
                    newDir = (from & LEFT);
